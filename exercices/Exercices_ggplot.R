...
dat %>%
  count(order)# how many observations in order

dat %>%
  count(family) # how many observations in family

dat %>%
  filter(!is.na(homerange)) %>%
  summarise(mean(homerange)) #what is the mean home range by family? Standard deviation? The sample size ?
...
## Graph 1 : Bar char of the number of entriers per family
